...
for key_name, key_value in required_keys.items():
    if key_value:
        st.sidebar.write(f"{key_name}: ✅ Present")
    else:
        st.sidebar.write(f"{key_name}: ❌ Missing")
        missing_any = True

# Store available keys in session state (no values shown)
if openai_env_key:
    st.session_state.openai_api_key = openai_env_key
if firecrawl_env_key:
    st.session_state.firecrawl_api_key = firecrawl_env_key
if perplexity_env_key and search_engine == "Perplexity AI - Sonar Pro":
    st.session_state.perplexity_api_key = perplexity_env_key
if exa_env_key and search_engine == "Exa AI":
